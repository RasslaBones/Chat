...
const rules = computed(() => {
  return {
    username: {
      ...
      // unique: helpers.withMessage("Username is already taken", submit),
    },
    email: {
...
const submit = () => {
  v$.value.$validate();
  if (!v$.value.$error) {
    store.dispatch("checkUsername", registerVals);
    router.push({ path: "/" });
  }
};
</script>
<template>
  ...
  >
    <div class="bg-gray-900 py-8 px-16 flex flex-col gap-4 rounded-md">
      <h1 class="text-gray-100 text-2xl text-center">Register a new account</h1>
      <RegisterInput
        title="Username"
